...
4. "Kya stock hai?": If asked what's in stock, list the items in the list.
`;
    }
              ...
              if (idMatch) {
                ...
                for (const fallbackUrl of fallbacks) {
                  try {
                    ...
                    if (!resp.ok) {
                      console.log(`Direct fetch failed (Tester) for ${fallbackUrl}, trying proxy...`);
                    ...
                    try {
                      ...
                      if (resp.ok) {
                        ...
                        if (text && text.length > 20 && !text.includes('<!DOCTYPE html>') && (text.includes(',') || text.includes(';') || text.includes('\t'))) {
                          setStockData(text);
                          console.log("Stock sync success (Tester) using Proxy for URL:", fallbackUrl);
                          return;
                        }
                      }
                    } catch(proxyErr) {}
